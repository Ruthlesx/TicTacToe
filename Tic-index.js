...
MYAPP.display = {
  ...
  showDrawMessages: function() {
    MYAPP.timeouts.push(
      setTimeOut(function() {
        $('.draw-message').fadeOut(500);
      }, 1500));
  },

  hideDrawMessages: function() {
    $('.draw-message').fadeout(1000);
  },

  showLoseMessages: function() {
    MYAPP.timeouts.push(
      setTimeout(function() {
        $('.losemessage').fadeIn(500);
      }, 1500)
    );
  },

  hideLoseMessages: function() {
    $('.lose-message').fadeOut(1000);
  },

  showWinMessages: function() {
    MYAPP.timeOuts.push(
      setTimeout(function() {
...
Math.floor(Math.random() * 2 + 1);
 ...
 $(this).text();
  ...
  playerTurn: function(square) {
    var symbol = MYAPP.turn === 1 ?
  MYAPP.playerOneSymbol :
  MYAPP.playerTwoSymbol;
   var box = 
   $(square).children('i').children('span');
    if(box.text() === '' && 
    MYAPP.gameInPlay && (MYAPP.turn === 1 ||
      (MYAPP.turn === 2 && 
        MYAPP.secondPlayer))) {
          box.text(symbol);
  ...
  computerPlay: function() {
    ...
    var boxNumber;
    if
    (computer.computerWhichMove(MYAPP.game) && MYAPP.turn === 2) {
      boxNumber = computer.computerWhichMove(MYAPP.game);
      var currentBox = $('.' + boxNumber).children('1');
      ...
      MYAPP.timeOuts.push(
        setTimeout(function() {
          ...
          MYAPP.game.updateSquare(boxNumber, 
          MYAPP.playerTwoSymbol);
           MYAPP.game.endTurn(symbol);
        }, 1000 ));
    }
  ...
  endTurn: function(symbol) {
    ...
    checkWin: function(symbol) {
      var currentBoard =
       MYAPP.currentBoard;
        var wins = MYAPP.winCombo;
        var winningCombo = [];
        var winner = 
      wins.some(function(combination) {
        var winning = true;
        for (var i = 0; i < 
        combination.length; i++) {
          if (currentBoard[combination[i]]
            !== symbol) {
              winning = false;
            }
        }
    ...
    reset: function() {
      ...
      MYAPP.timeOuts.push(
        ...
        //to not cause problem after first game
        setTimeout(function() {
          MYAPP.gameInPlay = true;
          MYAPP.game.play();
        }, 6000)
  ...
  firstPlay: function() {
    ...
    if (MYAPP.numFilledIn === 1) {
      //player plays center
      if (board[$] === MYAPP.playerOneSymbol) {
